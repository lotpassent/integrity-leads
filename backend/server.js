...
// ── EXPORTAR XLSX ─────────────────────────────────────────────────────────────
// ATENÇÃO: esta rota DEVE vir ANTES de /api/leads/:id
// Se vier depois, o Express lê "exportar" como valor de :id e nunca chega aqui
  ...
  try {
    ...
    if (leads.length > 0) {
      ...
      const respostasResult = await pool.query(
        `SELECT lead_id, campo, label, valor FROM lead_respostas
         WHERE lead_id = ANY($1) ORDER BY lead_id, id ASC`,
        [ids]
      );
      for (const r of respostasResult.rows) {
        if (!respostasMap[r.lead_id]) respostasMap[r.lead_id] = [];
        respostasMap[r.lead_id].push({ campo: r.campo, label: r.label, valor: r.valor });
      }
    }

    const leadsPayload = leads.map(r => ({
      ...r,
      criado_em:        r.criado_em     ? r.criado_em.toISOString()     : '',
      atualizado_em:    r.atualizado_em ? r.atualizado_em.toISOString() : '',
      respostas_extras: respostasMap[r.id] || [],
    }));

    const tmpJson = pathM.join(os.tmpdir(), `leads_${Date.now()}.json`);
    const tmpXlsx = pathM.join(os.tmpdir(), `relatorio_${Date.now()}.xlsx`);
    fs.writeFileSync(tmpJson, JSON.stringify(leadsPayload));
    const scriptPath = pathM.join(__dirname, 'generate_xlsx.py');

    function sendFile() {
      try { fs.unlinkSync(tmpJson); } catch {}
      if (!fs.existsSync(tmpXlsx)) {
        return res.status(500).json({ success: false, message: 'Arquivo não gerado.' });
      }
    ...
    execFile('python3', [scriptPath, tmpJson, tmpXlsx], { timeout: 30000 }, (err) => {
      if (err) {
        execFile('python', [scriptPath, tmpJson, tmpXlsx], { timeout: 30000 }, (err2) => {
          if (err2) return res.status(500).json({ success: false, message: 'Erro ao gerar planilha.' });
          sendFile();
        });
        return;
      }
...
app.get('/api/leads/exportar/csv', async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT id, consultor_nome, empresa_nome, empresa_setor, empresa_porte,
              contato_nome, contato_cargo, contato_email, contato_telefone,
              cargo_em_aberto, nivel_cargo, quantidade_vagas, urgencia, status,
              observacoes, contexto_entrevistado,
    ...
    const csv = [
      headers.join(';'),
      ...result.rows.map(r => [
        r.id, r.consultor_nome, r.empresa_nome, r.empresa_setor||'', r.empresa_porte||'',
        r.contato_nome||'', r.contato_cargo||'', r.contato_email||'', r.contato_telefone||'',
        r.cargo_em_aberto, r.nivel_cargo||'', r.quantidade_vagas, r.urgencia, r.status,
        (r.observacoes||'').replace(/;/g,','), (r.contexto_entrevistado||'').replace(/;/g,','),
...
app.delete('/api/leads/:id', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `DELETE FROM leads WHERE id = $1 RETURNING id, empresa_nome, cargo_em_aberto`,
      [req.params.id]
    );
...
app.get('/api/leads/:id/historico', async (req, res) => {
  try {
    ...
    if (!lead.rows[0]) return res.status(404).json({ success: false, message: 'Lead não encontrado.' });
...
app.get('/api/dashboard', async (req, res) => {
  try {
    const [totais, porStatus, porUrgencia, porConsultor, evolucao] = await Promise.all([
      pool.query(`SELECT
        COUNT(*) as total,
        COUNT(CASE WHEN status='fechado_ganho' THEN 1 END) as fechados,
        COUNT(CASE WHEN urgencia IN ('alta','critica') THEN 1 END) as urgentes,
        COUNT(CASE WHEN criado_em >= NOW() - INTERVAL '7 days' THEN 1 END) as esta_semana
        FROM leads`),
      ...
      pool.query(`SELECT DATE(criado_em) as data, COUNT(*) as total FROM leads
    ...
    res.json({
      ...
      metricas: {
        ...
        por_urgencia:     porUrgencia.rows,
        por_consultor:    porConsultor.rows,
